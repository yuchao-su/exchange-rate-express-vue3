<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="25%">
    ...
    >
      <el-form-item label="标签分组" prop="groupName">
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await UserTagGroupApi.getUserTagGroup(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as UserTagGroupApi.UserTagGroupVO
    if (formType.value === 'create') {
      await UserTagGroupApi.createUserTagGroup(data)
      message.success(t('common.createSuccess'))
    } else {
      await UserTagGroupApi.updateUserTagGroup(data)
