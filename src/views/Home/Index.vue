      ...
      <el-row :gutter="24" class="baseInfo">
        ...
        <el-col :xs="12" :sm="12" :lg="6" class="ivu-mb">
          <el-card :bordered="false" dis-hover :padding="12">
            ...
            <div class="content" v-if="count">
              <span class="content-number spBlock mb15"><count-to :start-val="0" :decimals="2" :end-val="count.priceCount" :duration="3200" class="card-panel-num" /></span>
              <el-divider />
              <div class="acea-row row-between-wrapper">
      ...
      <el-card shadow="never" class="mt-10px">
        <template #header>
          <div class="flex justify-between h-3">
            <span>{{ t('workplace.dataTrendChart') }}</span>
          </div>
        </template>
        <div class="echartBox">
          <Echart :height="350" :options="buryPointData" />
          <Echart :height="350" :options="storeOrderData" />
        </div>
      </el-card>
    </div>
    <div class="divBox">
...
import * as HomeApi from '@/api/system/home'
import PanelGroupT from './PanelGroupT.vue'
import { formatTime } from '@/utils'
import { useUserStore } from '@/store/modules/user'
import avatarImg from '@/assets/imgs/avatar.gif'
import {EChartsOption} from "echarts";
import {buryPointOptions, storeOrderOptions} from "@/views/Home/echarts-data";
import {set} from "lodash-es";
const buryPointData = reactive<EChartsOption>(buryPointOptions) as EChartsOption
const storeOrderData = reactive<EChartsOption>(storeOrderOptions) as EChartsOption
...
const getBuryPointChart = async () => {
  try {
    const data = await HomeApi.getBuryPointChart()
    console.log(data,'data')
    set(
      buryPointData,
      'title.text',
      t('workplace.userSessions')
    )
    set(
      buryPointData,
      'xAxis.data',
...
const getStoreOrderChart = async () => {
  try {
    const data = await HomeApi.getStoreOrderChart()
    console.log(data,'data')
    set(
      storeOrderData,
      'title.text',
      t('workplace.orderTotalAmount')
    )
    set(
      storeOrderData,
      'xAxis.data',
    ...
    set(storeOrderData, 'series', [
      {
        name: t('workplace.orderTotalAmount'),
        smooth: true,
        type: 'line',
        itemStyle: {},
        data: data.map((v) => v.value),
